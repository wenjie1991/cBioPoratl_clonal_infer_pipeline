...
sub MAIN ($output-fold, $sampleID, $dataset-fold, $config-yaml-temp = "./config/config.yaml.temp") {
    ...
    my $tables-file = $tables-fold ~ "mutation_cluster.tsv";
    ...
    pyclone-build-table($config-yaml-file, $tables-file);
...
## Build loci tables
sub pyclone-build-table ($config-yaml-file, $tables-file) {
    shell "PyClone build_table --config_file $config-yaml-file --out_file $tables-file --table_type loci";
}
